...
class Length {
	...
	/*
	 * WRITE CODE IN private static int length(int [] s, int x) YOU CANNOT USE ANY
	 * static variable in this function YOU can use as many local variables inside
	 * the function Cannot use any loop statements like for, while, do, while, go to
	 * Can use if. ONLY AFTER THE execution of this routine array s MUST have the
	 * same contents as you got it. YOU cannot call any subroutine inside this
	 * function except length itself (NOT length_easy)
	 */

	private static int length(int[] a, int l) {
	...
	private static int helper(int[] s, int x, int temp) {
		if (s[temp] == x)
			return 0;
		else {
			return 1 + helper(s, x, s[temp]);
		}
	}
